...
def test_compliant_license_is_returned(test_fedora_licenses):
    fake_pkg_data = {"info": {"classifiers" : [
        "Programming Language :: Python :: 3",
        "Programming Language :: Python :: 3.8",
        "Operating System :: POSIX :: Linux",
    ...
    pkg = PypiPackage("_", package_metadata=fake_pkg_data)
    assert pkg.license(check_compliance=True, licenses_dict=test_fedora_licenses) == "MIT AND MIT-0"


def test_bad_license_fails_compliance_check(test_fedora_licenses):
    fake_pkg_data = {"info": {"classifiers" : [
    ...
    with pytest.raises(SystemExit):
        pkg.license(check_compliance=True, licenses_dict=test_fedora_licenses)
...
def test_mix_good_bad_licenses_fail_compliance_check(test_fedora_licenses):
    fake_pkg_data = {"info": {"classifiers" : [
        "License :: OSI Approved :: European Union Public Licence 1.0 (EUPL 1.0)",
        "License :: OSI Approved :: MIT License",
    ]}}
...
def test_license_keyword_with_compliance_check(test_fedora_licenses):
...
def test_multiline_license_keyword_fails(compliant, test_fedora_licenses):
    fake_pkg_data = {"info": {"license": "This is absolutely\nhorrible\nidea\nto\ndo..."}}
    pkg = PypiPackage("_", package_metadata=fake_pkg_data)
    with pytest.raises(SystemExit):
        pkg.license(check_compliance=compliant, licenses_dict=test_fedora_licenses)


@pytest.mark.parametrize("compliant", (True, False))
def test_empty_license_keyword_fails(compliant, test_fedora_licenses):
