...
from pyp2spec.license_processor import is_compliant_with_fedora, good_for_fedora
...
def test_perl_license_raises_exception():
    with pytest.raises(NotImplementedError):
        license_keyword_to_spdx_identifiers("GPL-2.0-or-later OR Artistic-1.0-Perl")
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("Bitstream-Vera", False),  # real license, but non-existing in Fedora
    )
)
def test_compliance_check_with_fedora(identifier, expected, test_fedora_licenses):
    assert is_compliant_with_fedora(identifier, test_fedora_licenses) is expected
...
def test_is_allowed_in_fedora(identifiers, expected, test_fedora_licenses):
    """Test the function logic, bear in mind that Fedora status may change in time"""

    assert good_for_fedora(identifiers, licenses_dict=test_fedora_licenses) == expected
