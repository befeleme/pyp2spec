...
def classifiers_to_spdx_identifiers(classifiers):
    ...
    for classifier in classifiers:
        try:
            ...
            "If you believe that's pyp2spec's error, open an issue at project's GitHub repo." \
...
def license_keyword_to_spdx_identifiers(license_keyword):
    """Return a list of SPDX identifiers extracted from the license_keyword
...
def load_fedora_licenses(source_path=None, session=None):
...
def is_compliant_with_fedora(identifier, fedora_licenses):
...
def good_for_fedora(spdx_identifiers, licenses_dict=None, session=None):
    ...
    if licenses_dict is None and not FEDORA_LICENSES:
        FEDORA_LICENSES.update(load_fedora_licenses(session=session))
    ...
    for spdx_identifier in spdx_identifiers:
        if not is_compliant_with_fedora(spdx_identifier, fedora_licenses):
